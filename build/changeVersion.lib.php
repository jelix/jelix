...
class ChangeVersion {
    ...
    protected function changeJelixVersion($path, $minversion, $maxversion, $moduleVersion='') {
        ...
        $originalVersionRange = explode(',', $jelix->getAttribute('version'),2);
        $originalMin = '';
        $originalMax = '';
        if (count($originalVersionRange) > 0) {
            if (count($originalVersionRange) > 1) {
                if ($originalVersionRange[0][1] == '>') {
                    $originalMin = $originalVersionRange[0];
                    $originalMax = $originalVersionRange[1];
                }
                else {
                    $originalMin = $originalVersionRange[1];
                    $originalMax = $originalVersionRange[0];
                }
            }
            else {
                if ($originalVersionRange[0][0] == '>') {
                    $originalMin = $originalVersionRange[0];
                }
                else {
                    $originalMax = $originalVersionRange[0];
        ...
        if ($maxversion) {
            ...
            $versionRange .= $originalMax;
        }
        $jelix->setAttribute('version', $versionRange);
