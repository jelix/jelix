...
class UTDaoFactoryBase extends jUnitTestCaseDb {
    ...
    function setUpRun() {
        ...
        Mock::generatePartial('jDbConnection', 'MockjDbConnection', array('query', 'limitQuery', 'exec', 'lastIdInTable', 'tools', 'schema',
                                                                          ...
                                                                          '_doQuery','_doExec','_doLimitQuery'));
        Mock::generate('jDbResultSet');
    ...
    function setUp() {
        ...
        $this->conn->setReturnValue('query', $this->rs);
        $this->conn->setReturnValue('limitQuery', $this->rs);
        $this->conn->setReturnValue('exec', 0);
    }

    function testFindAll() {
        $dao = new cDao_testapp_Jx_products_Jx_mysql($this->conn);
        $this->conn->expectOnce('query', array('SELECT `products`.`id`, `products`.`name`, `products`.`price`, `products`.`promo` FROM `products` AS `products`'));
        $dao->findAll();
    }
    ...
    function testFindAllPrefix() {
        ...
        $dao = new cDao_testapp_Jx_products_Jx_mysql($this->conn);
        $this->conn->expectOnce('query', array('SELECT `products`.`id`, `products`.`name`, `products`.`price`, `products`.`promo` FROM `foo_products` AS `products`'));
        $dao->findAll();
    }
    ...
    function testGet() {
        ...
        $this->rs->setReturnValue('fetch', $o);
        $dao = new cDao_testapp_Jx_products_Jx_mysql($this->conn);
        $this->conn->expectOnce('query', array('SELECT `products`.`id`, `products`.`name`, `products`.`price`, `products`.`promo` FROM `products` AS `products` WHERE  `products`.`id` = 32'));
        $dao->get(32);
    }
    ...
    function testGetPrefix() {
        ...
        $this->conn->profile = array('table_prefix'=>'foo_');
        $dao = new cDao_testapp_Jx_products_Jx_mysql($this->conn);
        $this->conn->expectOnce('query', array('SELECT `products`.`id`, `products`.`name`, `products`.`price`, `products`.`promo` FROM `foo_products` AS `products` WHERE  `products`.`id` = 32'));
        $dao->get(32);
    }
    ...
    function testFindBy() {
        ...
        $this->conn->expectOnce('query', array('SELECT `products`.`id`, `products`.`name`, `products`.`price`, `products`.`promo` FROM `products` AS `products` ORDER BY products.price asc'));
        $dao->findBy($cond);
    }
    ...
    function testFindByPrefix() {
        ...
        $cond->addItemOrder('price', 'asc');
        // note: in the order clause, names are note enclosed between quotes because of the mock
        $this->conn->expectOnce('query', array('SELECT `products`.`id`, `products`.`name`, `products`.`price`, `products`.`promo` FROM `foo_products` AS `products` ORDER BY products.price asc'));
        $dao->findBy($cond);
    }
