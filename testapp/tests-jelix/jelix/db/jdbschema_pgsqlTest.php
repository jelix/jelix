...
class jDbSchema_pgsqlTest extends \Jelix\UnitTests\UnitTestCase {
    ...
    /**
     * @depends testDropReference
     *
     */
    public function testChangeColumnIdentityToSerial()
    {
        $db = jDb::getConnection('testapp_pgsql');
        $db->exec('DROP TABLE IF EXISTS city');
    ...
    longitude varchar(20))');
        ...
        $currentIdColDesc =   '<object class="jDbColumn">
            <string property="type" value="integer" />
            ...
            <integer property="length" value="0"/>
            <boolean property="sequence" value="false" />
            <string property="autoIncrementFlavor" value="GENERATED BY DEFAULT AS IDENTITY" />
        </object>';
     ...
     *
     */
    public function testChangeColumnIntegerToSerial()
    {
        $db = jDb::getConnection('testapp_pgsql');
        $db->exec('DROP TABLE IF EXISTS city');
        $db->exec('DROP SEQUENCE IF EXISTS city_city_id_seq');
