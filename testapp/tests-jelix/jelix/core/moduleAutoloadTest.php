...
class moduleAutoloadTest extends jUnitTestCase
{
    ...
    function testClassSection() {
        $conf = jApp::config();
        $this->assertEquals(10, count($conf->_autoload_class), '_autoload_class should have 5 declarations (for jelix_tests, jsoap, jacl, jacl2 modules)');
    ...
    function testNamespacePathMapSection() {
        $conf = jApp::config();
        $this->assertEquals(2, count($conf->_autoload_namespacepathmap), '_autoload_namespacepathmap should have 1 declaration');
