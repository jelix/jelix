...
class WebAssetsSelection {
    ...
    function addAssetsGroup($assetGroup) {
    ...
    function compute($config, $collectionName, $urlBasePath, $variables) {
        ...
        foreach($orderedSelection as $group) {
            $this->jsAssets = array_merge($this->jsAssets,
                $collectionAssets['webassets_'.$group.'.js']);
            $this->cssAssets = array_merge($this->cssAssets,
        ...
        switch($assetURLType) {
            ...
            case 'b': // relative path to base path with lang/locale
                return $this->urlBasePath.
                    str_replace(array('$lang', '$locale'),
                        array($this->variables['$lang'], $this->variables['$locale']), $resource);
            case 'a': // action
