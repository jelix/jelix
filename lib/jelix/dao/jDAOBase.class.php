...
abstract class jDAORecordBase {
   ...
   public function check(){
      ...
      foreach($this->_properties as $prop=>$infos){
         ...
         // test required
         if($infos['required'] && $value === null && $infos['type'] != 'autoincrement' && $infos['type'] != 'bigautoincrement'){
            $errors[] = array($prop,self::ERROR_REQUIRED);
            continue;
         }

         if($infos['type']=='varchar' || $infos['type']=='string'){
            if(!is_string($value) && $value !== null){
               $errors[] = array($prop,self::ERROR_BAD_TYPE);
               continue;
            }
            // test regexp
            if ($infos['regExp'] !== null && preg_match ($infos['regExp'], $value) === 0){
               $errors[] = array($prop,self::ERROR_BAD_FORMAT);
               continue;
            }
            ...
            if($infos['maxlength'] !== null && $len > intval($infos['maxlength'])){
               $errors[] = array($prop,self::ERROR_MAXLENGTH);
            }

            if($infos['minlength'] !== null && $len < intval($infos['minlength'])){
               $errors[] = array($prop,self::ERROR_MINLENGTH);
            }


         }elseif( in_array($infos['type'], array('int','integer','numeric', 'double', 'float'))) {
            // test datatype
            if($value !== null && !is_numeric($value)){
               $errors[] = array($prop,self::ERROR_BAD_TYPE);
               continue;
            }
         }elseif( in_array($infos['type'], array('datetime', 'time','varchardate', 'date'))) {
