...
gForm.setErrorDecorator(new jFormsErrorDecoratorAlert());    // declare an error handler
...
var jForms = {
    ...
    showHelp : function(aForm, aControl){
        var frm = this._forms[aForm.name];
        var ctrls = frm.controls;
        var ctrl = null;
        for(var i=0; i < ctrls.length; i++){
            if (ctrls[i].name == aControl) {
                ctrl = ctrls[i];
                break;
            }
        }
