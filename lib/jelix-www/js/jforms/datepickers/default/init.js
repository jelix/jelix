...
function jelix_datepicker_default(control,locale,basePath){
    ...
    $.include(this._config.basePath+'jelix/jquery/themes/default/ui.datepicker.css');
    this._jsFiles = [];
    ...
    this._loadJS = function (i){
        if(i<this._jsFiles.length){
            $.include(this._jsFiles[i], function(){ this._loadJS(i+1); });
        }
        else
            this._start();
    };
        ...
        jQuery.each(
            ...
            function(){
                ...
                if(control.multiFields){
                    ...
                    var eltYear = $(eltId+'_year').after('<input type="hidden" disabled="disabled" id="'+control.formName+'_'+control.name+'_hidden" />');
                    var eltMonth = $(eltId+'_month');
                    var eltDay = $(eltId+'_day');
                    var elt = $(eltId+'_hidden');
                }
                else
                    var elt = $('#'+control.formName+'_'+control.name);
    ...
    var me = this;
    $(document).ready(function(){ me._loadJS(0); });
}