...
var jForms = {
    ...
    declareForm : function(aForm){
        ...
        if(window.jQuery)
            $('#'+aForm.name).bind('submit',function (ev) { $(ev.target).trigger('jFormsUpdateFields'); return jForms.verifyForm(ev.target) });
        else{
...
jFormsControlDatetime.prototype.getValue = function(){
    if(!this.multiFields)
        return document.getElementById(this.formName+'_'+this.name).value;
