...
var jFormsJQ = {
    ...
    declareForm : function(aForm){
        ...
        if (elem.addEventListener) {
            elem.addEventListener("submit", function (ev) { if(!jFormsJQ.verifyForm(elem)) {ev.preventDefault(); ev.stopPropagation(); return false;} return true; }, false);
        } else if (elem.attachEvent) {
            elem.attachEvent("onsubmit", function (ev) { if(!jFormsJQ.verifyForm(elem)) { window.event.cancelBubble = true ; window.event.returnValue = false; return false;} return true;});
        }
    },
...
function jFormsJQControlSecretConfirm(name, label) {
    this.name = name;
    this.label = label;
    this.errInvalid = '';
    this.errRequired = '';
