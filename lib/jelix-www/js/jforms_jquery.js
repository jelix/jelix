...
var jFormsJQ = {
    ...
    declareForm : function(aForm){
        ...
        jQuery('#'+aForm.name).bind('submit',function (ev) { jQuery(ev.target).trigger('jFormsUpdateFields'); return jFormsJQ.verifyForm(ev.target) });
    },
...
jFormsJQControlDatetime.prototype.getValue = function(){
    if(!this.multiFields)
        return jQuery('#'+this.formName+'_'+this.name).val();
