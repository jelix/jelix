...
* @copyright    2008-2010 Julien Issler, 2008 Dominique Papin, 2011 Steven Jehannet
            ...
            function(data){
                ...
                if(elt.nodeType && elt.nodeName.toLowerCase() == 'select') {
                    ...
                    for (val in data.data) {
                        ...
                        else if(typeof data.data[val] == "object"){
                            var val2;
                            if(val == ""){
                                for(val2 in data.data[val]){
                                    opt = document.createElement('option');
                                    opt.setAttribute('value', val2);
                                    opt.appendChild(document.createTextNode(data.data[val][val2]));
                                    elt.appendChild(opt);
                                }
                            }
                            else {
                                optgrp = document.createElement('optgroup');
                                optgrp.setAttribute('label', val);
                                for(val2 in data.data[val]){
                                    opt = document.createElement('option');
                                    opt.setAttribute('value', val2);
                                    opt.appendChild(document.createTextNode(data.data[val][val2]));
    ...
    activate : function (val) {
        ...
        for(var j in this.items) {
            ...
            for(var i=0; i < list.length; i++) {
                ...
                console.log('readonly for '+list[i].name+':'+(list[i].readOnly?'tru':'false'));
