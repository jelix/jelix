...
var jFormsJQ = {
    ...
    declareForm : function(aForm){
        ...
        $('#'+aForm.name).bind('submit',function (ev) { $(ev.target).trigger('jFormsUpdateFields'); return jFormsJQ.verifyForm(ev.target) });
    },
...
jFormsJQControlDatetime.prototype.getValue = function(){
    if(!this.multiFields)
        return $('#'+this.formName+'_'+this.name).val();

    var controlId = '#'+this.formName+'_'+this.name;
    var v = $(controlId+'_year').val() + '-'
